...
function extractTextInParentheses(str) {
  ...
  const arr = [...matched].map((match) => match[1]);
...
function findWordOccurrences(str, word) {
  ...
  let matches = [];
  let match;
  // За допомогою циклу while створюємо
  // ітерацію поки рядок містить збіги з регулярним виразом, та змінній match присвоюємо збіги
...
function checkRegexFlags(regex) {
  ...
  const includeFlags = flags.includes("g") && flags.includes("m");
